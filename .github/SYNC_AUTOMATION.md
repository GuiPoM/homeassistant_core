...
## Key Differences from sync-integration-branch.yml

The old `sync-integration-branch.yml` only updated:
- Integration branch (rebase on upstream)
- Custom component (if requested)

The new `sync-all-branches.yml` updates:
- ✅ Dev branch (sync with upstream)
- ✅ Feature branches (rebase on upstream)
- ✅ Integration branch (rebuild from features)
- ✅ Custom component (sync from integration)

**Recommendation**: Use `sync-all-branches.yml` for complete maintenance.
